...
## 2026-06-11 — Context overflow (request 13086 > 8192 tokens)
...
## 2026-06-14 — Data layer cleanup + lean prompt + time-gap markers

- **memory.md removed (single source).** The parallel memMd mirror was written everywhere but read nowhere (engine = source since the wiring). Dropped `memMd`/`loadMd`/`saveMd`/`parseMd`/`buildMd` and all writes; all reads (known facts, relationships) now come from the engine via new `mem.byType()`/`mem.textsByType()`. Boot still reads a legacy memory.md once for one-time migration into the engine. Removed `memory.json.summary` (legacy). memory.json is now short-term/state only (recent + affection). Removed the memory.md edit button and md fields from `memory:get`. Roles now clean: settings.json (+optional persona.md) = identity, memory-items.json = long-term, memory.json = short-term/state, schedule.json = situation.
- **Lean system prompt (both languages).** The accumulated scaffolding (①–⑥ priority list, ★emphasis★, 6 acting rules, self-check, anti-recitation) made dialogue stiff/checklist-like and caused the model to recite time/place/memory every line (the self-check rule leaked because thinking is disabled). Rewritten to a short "you are X, here's your background, now text like a real person" prompt — background as context, one natural-talk instruction, minimal guardrails (stay her, no AI talk, current situation over earlier). 
- **Time-gap markers in history.** Root cause of "출근중 → 자려고 누움" contradictions: histMsgs fed the last N raw messages regardless of time, so a stale situational line from a previous slot ("자려고 누웠어") got treated as current. histMsgs now inserts a "(time has passed)" marker between messages more than 40 min apart, so old states read as past and the current grounding wins.
- Verified: `node --check` passed for both main.js and memory.js (mount synced this session).
